...
class ProfileTaskExecutionService:
    ...
    def add_profile_task(self, task: ProfileTask):
        ...
        if not success:
            logger.warning(
                "check command error, cannot process this profile task. reason: %s",
                error_reason,
            )
            return
        ...
        self.profile_task_scheduler.schedule(
            delay_millis, self.process_profile_task, [task]
        )

    def add_profiling(
        self, context: SpanContext, segment_id: str, first_span_opname: str
    ) -> ProfileStatusReference:
        logger.debug(
            f"========= add profiling, context:{context}, segment_id:{segment_id}"
        )
        execution_context = (
            self.task_execution_context.get()
        )  # type: ProfileTaskExecutionContext
        # 这里判断这个entrypoint是否在执行profiling
        logger.debug(f"====== execution_context is :{execution_context}")
        if execution_context is None:
            return ProfileStatusReference.create_with_none()

        return execution_context.attempt_profiling(
            context, segment_id, first_span_opname
        )
    ...
    def process_profile_task(self, task: ProfileTask):
        with self._rlock:
            ...
            if logger_debug_enabled:
                logger.debug(
                    "profile task [%s] for endpoint [%s] started",
                    task.task_id,
                    task.first_span_op_name,
                )

            millis = task.duration * self.MINUTE_TO_MILLIS
            self.profile_task_scheduler.schedule(
                millis, self.stop_current_profile_task, [current_context]
            )
    ...
    def stop_current_profile_task(self, need_stop: ProfileTaskExecutionContext):
        ...
        with self._rlock:
            ...
            if logger_debug_enabled:
                logger.debug(
                    "profile task [%s] for endpoint [%s] stopped",
                    need_stop.task.task_id,
    ...
    def _check_profile_task(self, task: ProfileTask) -> Tuple[bool, str]:
        try:
            ...
            if len(task.first_span_op_name) == 0:
                return (
                    False,
                    f"endpoint name [{task.first_span_op_name}] error, "
            ...
            if task.min_duration_threshold < 0:
                return (
                    False,
                    "min duration threshold must be greater than or equals zero",
                )
            ...
            if task.max_sampling_count >= ProfileConstants.TASK_MAX_SAMPLING_COUNT:
                return (
                    False,
                    f"max sampling count must be less than "
            ...
            with self._profile_task_list.mutex:
                for profile_task in self._profile_task_list.queue:  # type: ProfileTask
                    ...
                    if (
                        task.start_time
                        <= task_finish_time
                        <= self._cal_profile_task_finish_time(profile_task)
                    ):
                        return (
                            False,
