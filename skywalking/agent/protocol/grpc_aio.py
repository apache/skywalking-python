...
class GrpcProtocolAsync(ProtocolAsync):
    ...
    async def report_segment(self, queue: Queue, block: bool = True):
        start = None
        ...
        async def generator():
            ...
            while True:
                ...
                #             return
                #     segment = await wait_for(queue.get(), timeout) if block else queue.get_nowait()  # type: Segment
                # except QueueEmpty or TimeoutError:
                #     return
    ...
    async def report_log(self, queue: Queue, block: bool = True):
        start = None
        ...
        async def generator():
            ...
            while True:
                ...
                #         if timeout <= 0:  # this is to make sure we exit eventually instead of being fed continuously
                #             return
                #     log_data = await wait_for(queue.get(), timeout) if block else queue.get_nowait()  # type: LogData
                # except QueueEmpty or TimeoutError:
                #     return
    ...
    async def report_meter(self, queue: Queue, block: bool = True):
        start = None
        ...
        async def generator():
            ...
            while True:
                ...
                #         if timeout <= 0:  # this is to make sure we exit eventually instead of being fed continuously
                #             return
                #     meter_data = await wait_for(queue.get(), timeout) if block else queue.get_nowait()  # type: MeterData
                # except QueueEmpty or TimeoutError:
                #     return
                meter_data = await queue.get()
    ...
    async def report_snapshot(self, queue: Queue, block: bool = True):
        start = None
        ...
        async def generator():
            ...
            while True:
                ...
                #         if timeout <= 0:  # this is to make sure we exit eventually instead of being fed continuously
                #             return
                #     snapshot = await wait_for(queue.get(), timeout) if block else queue.get_nowait() # type: TracingThreadSnapshot
                # except QueueEmpty or TimeoutError:
                #     return
